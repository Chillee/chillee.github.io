...
So, you want to improve the performance of your deep learning model. How might you approach such a task? Often, folks fall back to a grab-bag of tricks that might've worked before or that they've seen on Twitter. "Use in-place operations! Set gradients to None! Install PyTorch 1.10.0 but not 1.10.1!"
...
When it comes to reasoning about whether your operation is memory-bandwidth bound, a calculator can go a long way.
...
This might be from Python (looking up attributes or dispatching to the right function) or code within PyTorch (all of PyTorch's [dispatcher](http://blog.ezyang.com/2020/09/lets-talk-about-the-pytorch-dispatcher/)). For example, when you do `a + b`, the following steps need to happen.
